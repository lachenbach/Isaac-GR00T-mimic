...
class So100DataConfigCustom(BaseDataConfig):
    video_keys = ["video.main_camera", "video.secondary_0"]
    state_keys = ["state.single_arm", "state.gripper"]
    action_keys = ["action.single_arm", "action.gripper"]
    language_keys = ["annotation.human.task_description"]
    observation_indices = [0]
...
class MimicFrankaConfig(BaseDataConfig):
    video_keys = [
        "video.workspace_view",
        "video.wrist_view_palm"
    ]
    state_keys = [
        "state.franka_eef_pos",
        "state.franka_eef_6d_rot", # could also try with the quaternion at some point
        "state.hand_joint_pos",
    ]
    action_keys = [
        "action.franka_eef_pos",
        "action.franka_eef_6d_rot",
        "action.hand_joint_pos",
    ]
    ...
    def transform(self):
        transforms = [
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    "action.franka_eef_pos": "min_max",
                    "action.hand_joint_pos": "min_max",
                },
            ),
            # concat transforms
            ConcatTransform(
...
DATA_CONFIG_MAP = {
    ...
    "so100_custom": So100DataConfigCustom(),
    "mimic_franka": MimicFrankaConfig(),
}
